...
// ResponseToLLMResponse converts a complete OpenAI Response to an ADK response.
// Both non-streaming replies and final streaming replies use this converter.
func ResponseToLLMResponse(resp *responses.Response, includeThoughts bool) (*model.LLMResponse, error) {
	converted, err := convertResponseWithThoughts(resp, includeThoughts)
	if err != nil {
		return nil, err
	}
	return GenerateContentResponseToLLMResponse(converted), nil
}

// GenerateContentResponseToLLMResponse converts the internal genai response
// representation used by stream deltas to ADK's LLMResponse.
func GenerateContentResponseToLLMResponse(res *genai.GenerateContentResponse) *model.LLMResponse {
	...
	return &model.LLMResponse{
		Content:       &genai.Content{Parts: []*genai.Part{}, Role: string(genai.RoleModel)},
		UsageMetadata: usage,
		ModelVersion:  res.ModelVersion,
	}
}
...
func convertResponse(resp *responses.Response) (*genai.GenerateContentResponse, error) {
	return convertResponseWithThoughts(resp, true)
}

func convertResponseWithThoughts(resp *responses.Response, includeThoughts bool) (*genai.GenerateContentResponse, error) {
	if resp == nil {
		return nil, ErrEmptyResponse
	}
	candidate, err := buildCandidateWithThoughts(resp, includeThoughts)
	if err != nil {
		return nil, err
	}
...
func buildCandidateWithThoughts(resp *responses.Response, includeThoughts bool) (*genai.Candidate, error) {
	parts, err := convertOutputItemsWithThoughts(resp.Output, includeThoughts)
	if err != nil {
		return nil, err
	}
...
func convertOutputItems(items []responses.ResponseOutputItemUnion) ([]*genai.Part, error) {
	return convertOutputItemsWithThoughts(items, true)
}

func convertOutputItemsWithThoughts(items []responses.ResponseOutputItemUnion, includeThoughts bool) ([]*genai.Part, error) {
	if len(items) == 0 {
	...
	for _, item := range items {
		...
		case "reasoning":
			signature, err := encodeReasoningSignature(item)
			if err != nil {
				return nil, err
			}
			thought := &genai.Part{Thought: true, ThoughtSignature: signature}
			if includeThoughts {
				var text []string
				...
				thought.Text = strings.Join(text, "\n")
			}

...
func TestConvertOutputItems(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.name == "valid items" {
				if len(parts[len(parts)-1].ThoughtSignature) == 0 {
					t.Fatal("reasoning part is missing its replay signature")
				}
				parts[len(parts)-1].ThoughtSignature = nil
			}
